...
### 출처 표기 (검증가능한 인용)
- 답변 본문에서 각 핵심 사실·수치·인용 **바로 뒤에** 근거 문서를 `[출처N]` 형식으로 인라인 표기하세요.
  (예: \"2026년 예산은 12억원입니다 [출처2].\")
- 한 문장에 여러 문서가 근거면 `[출처1][출처3]` 처럼 이어서 표기하세요.
- 답변 **맨 마지막 줄**에는 사용한 모든 문서 번호를 `[출처: 1, 3]` 형식으로 종합하세요.
- 문서 번호 N은 [문서N]의 N과 정확히 일치해야 하며, 실제로 답변에 사용한 문서만 포함하세요.";
