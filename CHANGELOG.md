...
문서 변환기 kordoc 4.14.4 번들. PDF 표·띄어쓰기·글자를 곡선으로 그린 PDF·OCR 을 더 정확히 읽습니다.

### 📄 문서 읽기 (kordoc 4.14.4)

- **[PDF] 띄어쓰기**: 한글이 글자 단위로 줄바꿈된 PDF 에서 "재 일학도의용군인"·"또 는" 처럼 낱말 중간에 공백이 들어가
  검색어와 맞지 않던 것을 고쳤습니다. 원본 한글 문서와 짝지은 PDF 417쌍 기준 어절 일치 0.925→0.971.
- **[PDF] 표 복원**: 쪽을 넘는 표의 쪼개진 행·반복 머리 행, 여러 쪽에 걸친 한 칸, 표 위에 걸친 그림·글상자를 바로잡았습니다.
  430쌍 1,784표 기준 정확히 일치하는 표 90→93%, 표 안의 표 64→72%.
- **[PDF] 한컴 밖에서 만든 PDF**: 예산서처럼 글자마다 떨어져 찍힌 숫자("2 0 , 7 7 5" → "20,775")와 조각난 괘선 표를 바로 읽습니다.
- **[OCR] 정확도**: 점선 괘선·색 칸·목차 점선을 바로잡아 글자 오류율 0.171→0.096, OCR 표 셀 일치 0.44→0.62.

### 🐛 수정

- **[검색] 글자를 곡선으로 그린 PDF**: 한글을 글꼴 대신 곡선으로 그린 쪽은 글자층에 한글이 없어 본문이 조용히 빠졌습니다.
  이제 이런 문서에도 "복사하면 글자가 깨질 수 있는 문서" 표시가 붙고, 결과 우클릭 "OCR로 다시 읽기"로 본문을 읽을 수 있습니다
  (kordoc 이 그 쪽의 실제 괘선으로 표를 복원).

### 🔧 기타

- 배포 빌드의 kordoc 고정 커밋을 v4.14.4 로 올렸습니다.

## [3.8.10] - 2026-09-24

v3.8.9 의 윈도우 설치본 빌드 수정. 기능은 3.8.9 와 같습니다(3.8.9 는 맥 설치본만 나갔습니다).

### 🔧 기타

- **[설치본] 윈도우 kordoc 번들 실패**: 번들 스크립트가 kordoc 런타임 의존성의 버전 범위(`^1.24.0`)를 `npm.cmd`
  명령줄로 넘겨, cmd.exe 가 `^` 를 이스케이프로 먹고 없는 버전(onnxruntime-node 1.24.0)을 찾다 실패했습니다.
  의존성을 번들 package.json 에 적고 인자 없이 설치합니다.

## [3.8.9] - 2026-09-24

기술부채 정리 릴리스. 문서 변환기 kordoc 4.14.3 번들(읽기 품질 전면 점검·그림 바이트 건너뛰기·상주 변환 워커),
검색·색인 결함 수리, 결과 목록·미리보기 개편, 화면 대비·글자 크기 정리.

### ⚡ 성능

- **[변환] 문서 속 그림 바이트를 받지 않음 (kordoc 4.14.3)**: 앱은 문서 속 그림을 쓰지 않는데
  kordoc 이 그림 바이트까지 뽑아 JSON 에 실어 보냈습니다. 그림이 많은 PDF 는 변환 결과가 200MB 에
  이르렀습니다. 이제 그림 자리 표시만 받습니다. 실측: 창원 계획서 PDF 출력 200→9.5MB·CPU 시간 30% 감소,
  강원 정보화 계획 출력 65→7.2MB·CPU 시간 20% 감소.
- **[변환] 상주 변환 워커 (kordoc 4.14.3)**: 파일마다 node 를 새로 띄우던 것을 워커 최대
  4개가 이어 받습니다. 실문서 27건: 색인 본문 27건 모두 같고 9.6→2.5초, 작은 문서는 건당 200~300ms →
  10ms 안팎. 구버전 kordoc·워커 실패 시에는 종전처럼 파일마다 실행하고, 300건 또는 메모리 1.5GB 를 넘긴
  워커는 교체하며, 2분 쉬면 내립니다. 미리보기는 워커가 모두 색인 중이면 기다리지 않습니다.
- **[검색] 스니펫을 상위 결과에만 계산**: 키워드 검색이 조건에 맞는 모든 청크의 스니펫을 만든 뒤
  잘랐습니다. 순위를 먼저 매겨 자르고 남은 결과에만 만듭니다 (중앙값 24% 단축).
- **[화면] 시작 JS 993→567KB**: 마크다운·수식 렌더러가 시작 번들에 섞여 있던 것을 분리했습니다.
- **[화면] 불필요한 다시 그리기 제거**: 키 입력마다 모든 결과 카드가, 색인 진행 이벤트마다 결과
  목록 전체가, 토스트·미리보기 폭이 바뀔 때마다 결과 카드의 우클릭 메뉴가 다시 그려지던 것을
  막았습니다. 미리보기 경계 드래그는 프레임당 한 번만 반영하고, AI 답변 마크다운은 한 번만 파싱합니다.
- **[미리보기] 같은 문서 다시 열 때 변환 생략**: 경로·수정시각·크기가 같으면 최근 16개(64MB)를
  캐시에서 씁니다.

### 📄 문서 읽기 (kordoc 4.14.3)

- **[PDF] 표 복원**: 쪽을 넘는 표를 잇고 표 안의 표를 칸 안에 넣습니다. 한글 문서와 짝지은 PDF 430쌍 1,784표 기준
  정확히 일치하는 표 64→90%.
- **[한글] HWP 표를 HWPX 와 같은 규칙으로**: hwp↔hwpx 짝 3,593표가 모두 같게 읽힙니다(82→100%). 머리말·꼬리말·각주·캡션 안
  표가 빠지던 것, HWP3 표 격자, HWPX 각주·미주 번호를 고쳤습니다.
- **[OCR] 기울어진 스캔**: 기울기를 보정하고 세로로 이어진 글 상자를 나눠 읽습니다. 글자 오류율 0.201→0.171, 3° 기울어진
  스캔 57.6→22.0%.
- **[한글] 본문의 `$` 가 미리보기에서 수식으로 바뀌던 것**("echo $HOME $PATH"), **미기입 누름틀 안내문이 본문처럼 읽히던 것**
  (한컴이 인쇄하지 않는 흐린 안내문)을 고쳤습니다.

### 🐛 수정

- **[색인] 오프라인 공유 폴더 기록 삭제**: 앱 시작 정리가 잠깐 안 보이는 네트워크 폴더의 문서
  기록과 북마크를 지웠고, 폴더 동기화는 읽지 못한 하위 폴더(권한 없음·연결 끊김)의 문서를 지웠습니다.
  둘 다 파일이 없다고 확인된 경우만 지웁니다.
- **[AI 검색] 다른 문서의 벡터가 붙던 문제**: 지워진 청크의 벡터가 남은 채 청크 번호가 재사용되면
  엉뚱한 문서 벡터가 붙었습니다. 시작 시 DB 에 없는 벡터를 정리하고, 정리·삭제 때 벡터도 함께 지웁니다.
- **[색인] 윈도우 삭제 감지·이름 바꾸기·폴더 삭제**: 폴더 경로 패턴이 `\%` 리터럴이라 윈도우에서
  삭제된 파일이 인덱스에 남았고, 다른 곳으로 옮긴 파일과 통째로 지운 폴더도 반영되지 않았습니다.
  감시 폴더 안으로 옮겨 온 폴더의 문서는 다음 동기화까지 기다리지 않고 바로 색인합니다.
- **[색인] 취소가 먹지 않던 문제**: 첫 스캔 중 취소해도 본문 색인이 이어지고, 사이드바가 취소한
  폴더를 다시 시작했습니다. 문서가 많을 때 취소하면 색인 작업이 끝나지 않고 파일 감시도 멈춘 채
  남던 것도 고쳤습니다.
- **[색인] 한 문서 저장 실패가 배치 전체를 되돌림**: 문서별 SAVEPOINT 로 격리했습니다. 파싱 실패로
  메타데이터만 남길 때 옛 청크와 벡터가 검색에 남던 것도 지웁니다.
- **[색인] 엑셀 빈 칸이 많은 시트에서 앱 종료**: 멀리 떨어진 칸 하나 때문에 거대한 배열을 잡던 것을
  칸 단위 스트리밍으로 바꿨습니다.
- **[검색] 제외어(-단어)가 청크 단위로만 걸림**: 제외어가 든 문서는 문서째 뺍니다. 파일당 3개 제한보다
  청크 LIMIT 가 먼저 걸려 결과가 줄던 것, 버전 묶음이 가장 낮은 결과를 대표로 쓰던 것, 파일명
  검색 범위가 이름이 비슷한 옆 폴더까지 잡던 것, É·Д 같은 비영문 대문자가 든 폴더로 범위를 좁히면
  결과가 비던 것도 고쳤습니다.
- **[미리보기] 암호 문서에 암호 입력이 안 뜸**: kordoc 이 실패 코드와 함께 비정상 종료하면 코드가
  버려졌습니다.
- **[미리보기] 표 셀의 `<개정 2024.1.1>` 같은 글자가 사라짐**: 표를 글자로 풀 때 태그로 오인했습니다.
- **[OCR] 띄어쓰기 누락·줄 순서 뒤섞임**: 인식기의 공백 문자를 버리고, 정렬 비교가 일관되지 않았습니다.
- **[감시] 시작 직후 파일 감시가 꺼져 있던 문제**, 일시정지 횟수가 어긋나 감시가 영구 정지되던 문제,
  감시 스레드가 패닉 한 번에 멈추던 문제를 고쳤습니다.
- **[안정성] 특수 유니코드 검색어·긴 오류 로그·모델 동시 다운로드·열기/폴더 열기 좀비 프로세스** 등
  패닉과 누수 여러 건.
- **[화면] Anything 모드에서 폴더 수가 1개↔2개로 바뀌면 앱 전체가 오류 화면으로 바뀌던 문제**
  (검색 범위 칩의 훅 순서), 스마트 검색 조건 칩의 ✕ 가 아무것도 안 하던 문제, 저장되지 않는
  "결과 1000개" 옵션을 고쳤습니다.

### ✨ 화면

- **[결과] 그룹 보기 키보드 이동**: ↑↓ 가 파일 단위로 움직이고 선택한 카드가 보입니다. 파일명
  컬럼을 정렬한 목록도 화면 순서대로 움직입니다.
- **[결과] 빈 화면이 이유와 다음 행동을 알려 줌**: 첫 폴더를 읽는 동안은 진행 상황(N / M)을,
  필터에 결과가 가려지면 가려진 개수와 [필터 풀기]를 보여 줍니다. 종전엔 둘 다 "결과 없음"이었습니다.
- **[결과] 긴 파일 이름은 가운데를 줄임**: `…_최종_수정본.hwpx` 처럼 끝이 보입니다.
- **[미리보기] 창 폭이 바뀌어도 다시 읽지 않음**: 좁은 창(겹쳐 띄우기)과 넓은 창(옆에 붙이기)을
  오갈 때 문서를 다시 읽고 스크롤·요약·찾기가 날아가던 것을 고쳤습니다. 오류 화면에 [다시 시도]
  [원본 열기], 글자 없는 문서에 [OCR로 다시 읽기]를 둡니다.
- **[오류] 알아들을 수 있는 오류 문구**: 백엔드 오류 코드 22종을 모두 우리말 문구로 바꿉니다.
  "[object Object]" 나 경로만 덩그러니 뜨던 곳을 고쳤습니다.
- **[접근성] 대비·키보드·화면 낭독기**: 강조 초록을 #01AF7A→#017F58 로(글자 4.8:1, 흰 글씨 5.0:1),
  흐린 글자·경고·정보·AI 색을 WCAG AA 로 올렸습니다. 다크 모드의 초록 버튼은 어두운 글씨를 씁니다.
  사이드바 목록·프리셋·참조 문서는 진짜 버튼이 되어 Tab 으로 닿고, 설정 토글은 이름을 읽습니다.
- **[글자] 11px 미만 글자 없음**: 하드코딩 글자 크기 147곳을 디자인 눈금으로 바꿨습니다
  (`text-2xs`=11px 신설). 한글 라벨의 대문자·넓은 자간을 뺐습니다.
- **[확인] 되돌리기 어려운 동작은 한 번 묻기**: 폴더 제거, 다시 읽기, 전체 드라이브 인덱싱.
  실패는 늘 알림으로 알려 줍니다.
- **[검색창] 지우기(×) 버튼**, 축소 검색바에서 질문 모드 Enter 전송, 필터가 두 줄로 겹쳐 나오던 것 정리.
- **[문구] 줄표·말줄임 정리, 맥에서 "탐색기"→"Finder"·단축키 ⌘ 표기**, 파일 형식 필터를 "HWPX, DOCX" 대신
  "한글, 워드"로, AI 없는 설치본의 AI 문구 제거.

### 🔧 기타

- **[보안] rustls 0.23.44→0.23.45**: TLS 1.3 핸드셰이크 메시지를 암호화 단계 경계를 넘어 받아들이던 문제
  (RUSTSEC-2026-0285). 앱 코드 변경은 없습니다.
- **[설치본] 크기 축소**: 다른 OS 용 ONNX 런타임 바이너리와 onnxruntime-web 을 번들에서 뺍니다.
  kordoc 런타임 의존성 목록은 kordoc package.json 한 곳에서 읽습니다. 맥 번들 Node v22.23.2.
- **[개발] 1,200줄 넘던 파일 분리**: kordoc 연동·색인 파이프라인·DB·앱 시작(Rust), 미리보기·결과 목록(React).
- **[개발] 하니스 복구·확장**: 우클릭 "Markdown으로 저장" 추가 뒤 결과 목록 하니스가 멈춰 있던 것을 고치고, 키보드 이동·
  가운데 말줄임 검증을 더했습니다 (뷰어 38·결과 36·파일명 컬럼 28 통과).
- **[개발] DESIGN.md 현행화**: 색·글자 눈금·대비 규칙·문구 규칙.

## [3.8.8] - 2026-09-22

의미 검색이 개인정보 마스킹 별표(`*******`)에 끌려가던 문제 수정.

### 🔧 변경

- **[검색] 마스킹 별표 런을 인덱스에서 접음**: 정보공개포털 결재문서 등은 이름·인원·금액을
  `*` 로 가려 내보내는데, 이 별표가 임베딩 토큰 예산을 삼켰습니다 (31자 마스킹 하나가
  64토큰 — 제목 한 줄이 13토큰). 마스킹 필드 서너 개면 청크의 절반이 별표라 문서 벡터가
  내용과 무관하게 뭉개져, "상봉동 민원 현장확인 계획" 같은 검색이 34등까지 밀렸습니다.
  인덱싱 시 3개 이상 이어진 별표(kordoc 이스케이프 `\*` 포함)를 공백 하나로 접습니다.
  결재문서 50건·검색어 44개 실측: 벡터 검색 1위 적중 77%→91%, 3위 내 80%→98%.
  전 파서 공통, 미리보기는 원본 그대로. 이미 인덱싱된 문서는 재인덱싱해야 반영됩니다.
- **[개발] 검색 품질 회귀셋 신설** `harness/search-quality/`: 실제 공공문서 50건 + 검색어
  44개로 벡터 검색 top-k 적중률을 재는 벤치. 인덱싱 파이프라인을 파이썬으로 재현해
  청크·정규화 변경의 전후 차이를 잡습니다 (위 수정이 첫 사례).

## [3.8.7] - 2026-09-20

검색 결과 "파일 열기" 의 확장자 제한 제거 (#46 후속).

### 🔧 변경

- **[열기] 검색된 파일은 종류와 무관하게 기본 앱으로 열림**: v3.8.6 까지 `open_file` 은
  문서·이미지 19종 허용 목록만 통과시켜 `.java`·`.json`·`.xml` 같은 파일명 검색 결과가
  "지원하지 않는 파일 형식" 으로 거부됐습니다. 찾은 파일을 못 여는 건 탐색기·Everything 등
  다른 검색 앱과 비교해 부자연스럽다는 지적(#46)에 따라 확장자 검증을 없애고, 탐색기처럼
  실행 파일까지 그대로 엽니다. 감시 폴더 스코프 검증·시스템 보호 폴더 차단은 그대로.
- **[열기] 실패 토스트에 거부 사유 표시**: "파일 열기 실패" 만 보여 원인을 알 수 없던 것을
  백엔드 사유(감시 폴더 외부·시스템 폴더 등)와 함께 표시.
...
- **[검색] 문서가 적으면 키워드 검색이 본문 0건 (파일명 매치만 표시)**: 키워드
  confidence 가 bm25 절대 크기(`|best|/5`)로 감쇠됐는데, bm25 크기는 IDF 에 종속돼
  코퍼스가 작거나(실측: 문서 8개) 검색어가 문서 절반 이상에 등장하면 FTS5 가 IDF 를
  1e-6 으로 클램프해 스코어가 전부 0.0 으로 붕괴 → 모든 결과 0% → `min_confidence`
  (설정 "최소 신뢰도", 권장값 20~30) 필터가 실제 매치를 전부 숨겼습니다. 파일명 매치는
  이 필터를 안 타서 "파일명은 뜨는데 본문은 결과 없음"으로 보이고, 본문이 비니 그룹
  보기도 비어 보였습니다. 문서가 많아지면 IDF 가 살아나 저절로 "고쳐진" 것처럼
  보이던 현상. 이제 키워드 confidence 는 결과 집합 내 상대 순위만 뜻합니다(최상 100,
  최하 0, 전원 동점 100) — 키워드 % 는 UI 에 노출되지 않으므로 필터·정렬에만 영향.
  `normalize_fts_confidence` 회귀 테스트 3건 추가.

## [3.8.4] - 2026-09-05

kordoc 사이드카 v4.7.3 → v4.12.0 갱신. 앱 코드 변경 없이 번들 파서 런타임만 올린 유지보수 릴리스.

### 🔧 변경

- **[파서] kordoc 사이드카 v4.12.0** (publish.yml SHA 핀 `b7b2254` → `de18262`): HWP3
  그리기 개체 안 글상자 텍스트 회수(kordoc #73), HWP3 완성형 좌표 규칙 누락 수정(#72),
  `--format json` 페이지별 마크다운(#68), 실패 JSON 전 포맷 확장(#69), 이미지 저장 시
  `images/manifest.json`(#70). 앱은 이미 `--format json --silent` 의 실패 JSON `code` 를
  읽고 있어 호출 계약 변경 없음.
...
- **우클릭 'Markdown으로 저장'**: 파일명 검색을 포함한 모든 검색 결과의 우클릭 메뉴에서
  파싱된 문서 내용을 `.md` 파일로 내보냅니다. 지금까지는 미리보기 패널을 연 뒤 ⋯ 메뉴에서만
  가능했고, 미리보기를 거치지 않는 파일명 검색 결과에는 내보내기 경로가 없었습니다.
  - 파서 지원 문서 포맷(hwp·hwpx·docx·pptx·xlsx·xls·pdf·txt·md·eml)에만 메뉴가 나타나며,
    이미지·압축 등 파싱 불가 포맷에는 표시되지 않습니다.
  - 미리보기와 같은 파싱 경로(kordoc 직접 추출, 실패 시 검색 인덱스 fallback)를 사용합니다.
    파싱된 내용이 없으면 저장하지 않고 알려주며, 암호 문서는 미리보기에서 암호를 입력한 뒤
    저장하도록 안내합니다.
...
- **설치된 MS Office 로 인덱싱** (#41, [@ssethj](https://github.com/ssethj) 기여, Windows 전용·기본 off):
  자체 보안 솔루션이 문서를 암호화하는 사내 환경에서는 허가된 프로그램 외에는 파일이
  깨진 것으로 보여 kordoc·OOXML 직접 파싱이 전부 실패합니다. 이때 **회사가 공식 허가한
  MS Office 자체를 COM 자동화로 띄워** 본문을 읽어 인덱싱합니다. 설정 > 검색 탭에서
  Word(DOCX)·Excel(XLSX/XLS)·PowerPoint(PPTX) 를 각각 켤 수 있고, **기존 파서가 실패한
  파일에만** 동작하는 최후 fallback 입니다. Office 창은 화면에 뜨지 않으며, 열기 시
  매크로 자동실행을 차단(`AutomationSecurity=ForceDisable`)합니다.
- **밑줄 보존** (kordoc v4.7.x): PDF·HWPX·HWP5 원문의 밑줄 강조가 미리보기에 그대로
  살아납니다. 공문의 부서명·각주·절 제목 강조가 밋밋해지던 부분입니다.
- **PDF 링크 추출** (kordoc v4.7.1): PDF 안의 하이퍼링크가 `[표시문구](주소)` 로 남아
  본문과 함께 검색됩니다.

### 🐛 수정

- **MS Office fallback 3건** (위 #41 후속, 릴리스 전 수정):
  - **설정 토글이 서로 뒤바뀌어 있던 문제** — PowerPoint 토글을 켜면 XLSX 가, Excel
    토글을 켜면 PPTX 가 fallback 되어 정작 켠 포맷은 동작하지 않았습니다.
  - **사용자가 쓰던 Word/Excel 이 강제 종료되던 문제** — Word·PowerPoint 는 단일 인스턴스
    자동화 서버라 앱이 이미 떠 있으면 그 인스턴스에 붙습니다. 인덱싱이 끝날 때마다 무조건
    `Quit` 을 호출해 **사용자가 편집 중이던 문서까지 (경고를 꺼 둔 탓에 저장 확인도 없이)
    닫힐 수 있었습니다.** 이제 열린 문서가 하나도 없는 인스턴스만 종료합니다.
  - **Office 전역 설정이 되돌아오지 않던 문제** — `Visible=false`·`ScreenUpdating=false`·
    `EnableEvents=false` 를 사용자 인스턴스에 걸어 둔 채 끝나면 Word 창이 사라지거나
    Excel 매크로 이벤트가 죽습니다. 바꾼 값은 전부 원래대로 복원합니다(RAII 가드).
- **검색 스니펫에 `<u>` 태그가 노출되던 문제** (kordoc v4.7.0 반영 후속): 밑줄은 인라인
  HTML 로 오므로 검색용 본문에서는 걷어냅니다 — 낱말 중간에서 열리는 태그라 공백이 아니라
  삭제로 처리해 형태소 토큰이 쪼개지지 않습니다. 미리보기는 원본 그대로 밑줄을 렌더합니다.
- **2단 지면 읽기 순서**·**BOM 이 붙은 OOXML**·**이미지 다량 HWP 의 빈 출력** (kordoc
  v4.6.x): 시험지·잡지형 2단 문서에서 좌우 단이 섞이던 문제, 일부 라이터가 만든 xlsx/docx 가
  통째로 파싱 거부되던 문제, 이미지가 수백 장인 HWP 가 본문 없이 인덱싱되던 문제가 함께 해소됩니다.

### 📦 의존성

- kordoc 사이드카 핀 **v4.4.0 → v4.7.1** (`4b65e57`, publish.yml 두 job 동시 갱신).

### ✅ 검증

로컬 게이트 전부 통과 (tsc+vite 빌드 · cargo fmt/clippy · **테스트 290 pass**).
kordoc 밑줄 방출은 실제 코퍼스로 확인 — PDF 15문서 중 6문서, HWPX 20문서 중 2문서에서
표 바깥 본문에 `<u>` 가 실제로 방출됨을 확인하고 그 경로를 막았습니다.
COM 코드는 `#[cfg(windows)]` 라 macOS 에서 컴파일되지 않습니다 — Windows CI 가 최종 검증합니다.
...
내부망 PC 에서 보안솔루션(안랩 V3·ZombieZERO 등)이 앱을 격리하거나 기동을 막던 문제를,
개별 증상마다 우회하는 대신 **탐지 표면 자체를 컴파일 단계에서 들어낸 별도 빌드**로 해결합니다.
같은 소스에서 Cargo feature 하나(`online`)를 빼고 빌드하므로 코드가 갈라지지 않습니다.

### ✨ 추가

- **Anything Lite 설치본** (`Anything Lite_3.7.0_x64-setup.exe`): 릴리스에 함께 첨부됩니다.
  내부망·폐쇄망 PC 용이며, 인터넷이 되는 PC 는 기존 `Anything_3.7.0_x64-setup.exe` 를 쓰시면 됩니다.
  - **네트워크 코드가 물리적으로 없습니다.** HTTP 클라이언트(`ureq`)를 링크조차 하지 않아
    바이너리에 HTTP/TLS 스택과 외부 호스트 문자열이 남지 않습니다. IT 부서가 `strings` 검사로
    직접 확인할 수 있고, CI 가 릴리스마다 이를 자동 검증합니다.
  - 제외: 실행 중 모델/DLL 다운로드, Telegram 오류 리포트, 자동 업데이트, GitHub 릴리스 조회,
    AI 질의응답·요약, AppData 로의 실행파일(PE) 복사.
  - 제거한 시스템 호출: `powershell.exe -EncodedCommand`(디스크 유형 조회 → 드라이브 추정으로 대체),
    `rundll32.exe url.dll`(URL 열기 → `explorer.exe` 로 대체), 진단용 `LoadLibraryEx` 직접 호출.
    셋 다 정상 용법이지만 EDR 이 각각 인코딩 PowerShell·LOLBin(T1218.011)·DLL 사이드로딩으로
    분류하는 호출입니다.
  - `onnxruntime.dll`·`pdfium.dll`·OCR 모델을 번들하지 않습니다 → **동적 DLL 로드 시도가
    프로세스 생애에 한 번도 일어나지 않습니다.** kordoc 번들에서도 수식 OCR 용 네이티브
    바이너리 4종을 뺐습니다.
  - 기존 `settings.json` 을 그대로 가져오거나 파일을 직접 고쳐도 시맨틱·OCR·AI 가 되살아나지
    않습니다(로드·저장 양쪽에서 강제). 설정·DB 는 일반 배포본과 분리 저장됩니다.
  - **남는 기능**: 전문 검색(FTS5 + 한국어 형태소·근접 검색·검색 연산자), 파일명 검색,
    HWP/HWPX/DOCX/XLSX/PDF/이미지·`.eml` 파싱과 미리보기, 실시간 감시·증분 인덱싱,
    문서 비교, 버전 계보, 태그·북마크, 중복 찾기, 내보내기.
  - **빠지는 기능**: 시맨틱/하이브리드 검색, OCR, AI 질의응답·요약, 자동 업데이트, 오류 자동 리포트.
  - 배경·제외 목록·IT 부서 예외 등록 대상은 [docs/LITE-BUILD.md](docs/LITE-BUILD.md) 참고.

### 🐛 수정

- **`scripts/bump-version.ps1` 이 실행조차 되지 않던 문제**: `"...v$Version:"` 을 PowerShell 이
  드라이브 한정 변수로 파싱해 스크립트 전체가 ParserError 로 죽었습니다.
- **같은 스크립트가 `Cargo.toml` 버전을 조용히 건너뛰던 문제**: 정규식에 `(?m)` 이 없어 `^` 가
  파일 선두에만 걸렸습니다. `package.json` 만 올라간 채로 `release.sh` 의 버전 일치 검사를
  통과해, 바이너리 버전만 옛 값으로 배포될 수 있는 경로였습니다.

### 📝 알려진 제한

- **코드 서명은 여전히 미적용입니다** (`certificateThumbprint: null`). 서명 없는 실행 파일이라는
  사실이 나머지 모든 탐지 항목의 심각도를 키웁니다. Lite 는 탐지 표면을 최소화할 뿐 서명을
  대신하지 못합니다 — OV/EV 코드서명 인증서 도입이 남은 과제입니다.
- Lite 도 `node.exe`(kordoc 사이드카, HWP 파싱 필수)와 `msedgewebview2.exe`(UI 렌더러)는
  자식 프로세스로 띄웁니다. 차단하는 정책이 있으면 예외 등록이 필요합니다.

## [3.6.1] - 2026-08-04

**v3.6.0 암호 미리보기 수리(kordoc 핀 드리프트) + AI 답변 MD 저장 + AI 안내 모달 provider 분기**

### 🐛 수정

- **v3.6.0 암호 문서 미리보기가 배포본에서 동작하지 않던 문제**: 릴리스 빌드가 kordoc v4.3.0을 번들하고 있었습니다(publish.yml SHA 핀 미갱신). 암호 입력 시 kordoc CLI가 알 수 없는 `--password` 옵션으로 실패했습니다. 핀을 v4.4.0(e92525f)으로 갱신 — HWP3 아래아 소실·한컴 PUA 11종·CDATA 텍스트 소실 수정도 함께 반영됩니다.
- **AI 안내 모달이 로컬·사내 LLM 설정에서도 "Google Gemini API로 전송"으로 표시** (#37): 모달이 열릴 때 현재 provider 설정을 읽어 문구를 분기합니다. OpenAI 호환 endpoint는 설정한 base URL을 표시하고, localhost/루프백이면 "이 컴퓨터 안에서만 처리됩니다"로 안내합니다.

### ✨ 추가

- **AI 답변을 Markdown 파일로 저장** (#39, @amnotyoung): AI 답변 하단 액션 바에 "MD 저장" 버튼 추가. 질문·답변·참조 문서를 한 문서로 묶어 저장하며, 저장 다이얼로그는 질의에 사용한 검색 범위 폴더에서 열립니다.

### 📝 문서

- README 개발자용 섹션에 kordoc 번들 빌드 단계 추가 (#40, @tindone)
- README macOS 설치 안내에 macOS 15 Sequoia Gatekeeper 팝업("악성 코드가 없음을 확인할 수 없습니다") 대응 방법 추가 (#38)

## [3.6.0] - 2026-08-01

**암호로 보호된 문서 열기 — 미리보기에서 비밀번호 입력 (kordoc v4.4.0)**

### ✨ 추가

- **암호 문서 미리보기**: 열기 암호가 걸린 HWPX·HWP·HWP3 문서를 미리보기에서 비밀번호를 입력해 열 수 있습니다. 그동안은 "암호로 보호된 파일"로 차단되어 내용을 볼 수 없었습니다.
  - 세 포맷의 암호화 방식이 각각 다릅니다 — HWPX는 ODF 표준(AES-256-CBC + PBKDF2), HWP3는 DES-ECB, HWP5는 EncryptVersion 4의 비트 단위 CFB.
  - 틀린 암호는 "성공했는데 내용이 이상한" 결과 대신 명확히 실패로 알립니다.
  - 암호는 그 문서를 여는 데만 쓰고 저장하지 않습니다. 인덱싱 경로는 종전대로 암호 문서를 건너뜁니다.

### 🐛 수정 (kordoc v4.4.0 반영)

- **HWP3 본문 대량 유실**: 그림 뒤 캡션 문단을 읽지 않아 그림 하나당 스트림이 어긋났고, 어긋난 자리가 "문단 끝"으로 읽히면 경고 없이 나머지를 통째로 버렸습니다. 945KB 문서가 0자로 나오던 수준입니다(최대 100만 자 복구).
- **CDATA 텍스트 소실**: `<![CDATA[...]]>`로 저장된 문단이 통째로 사라지던 문제 — HWPX·HWPML·DOCX 세 포맷에 걸쳐 있었습니다.
- **사라지던 글자 복원**: 결재란 `(인)`, 편람 글머리표, 사각 안 숫자 ①~⑳ 등 한컴 전용 기호 11종과 HWP3 옛한글 아래아(`ᄒᆞᆫ글`).
- **양식 채우기**: 같은 이름의 누름틀이 여러 곳에 있는 서식에서 한 곳도 채워지지 않던 문제, 빈 셀 채우기가 엔티티 원문을 지우던 문제.
...
이슈 #35 는 v3.4.7 로 해결됐지만(사용자 로그: 워밍업 185ms, `engine initialized dict=11945`),
그 체인을 되짚어 보니 같은 뿌리에서 나온 문제가 몇 가지 남아 있었습니다.

### 🐛 수정

- **pdfium 을 매 부팅 지웠다 다시 쓰던 문제 해소** — 번들 자산을 `models/` 로 복사하는 seed 는
  "이미 최신이면 skip" 하도록 돼 있었는데, 해시를 고정하지 않은 자산(pdfium·macOS dylib)은
  비교 대상이 빈 문자열이라 **판정이 항상 거짓**이었다. 그래서 실행할 때마다 7.2MB 를 삭제하고
  다시 썼다(이슈 #35 로그에 `seed: 번들 pdfium dylib 적용 완료` 가 매번 찍힌 게 그 증거).
  v3.4.4 부터 로드 경로는 설치본 번들이라 이 사본은 쓰이지도 않는데, "프로세스가 방금 디스크에
  쓴 실행 파일" 이라는 **보안솔루션 dropper 오탐 패턴만 남기고 있었다.** 크기 비교로 판정을
  바꿔 재복사를 없앴다 — 개행 변환 오염이나 버전 교체는 크기가 바뀌므로 그대로 잡힌다.
- **'OCR로 다시 읽기' 가 앱 전체를 멈출 수 있던 경로 제거** — 이 명령만 여전히 **컨테이너 락을
  쥔 채** OCR 엔진을 동기 초기화하고 있었다(v3.4.5 가 다른 경로에서 없앤 패턴). ONNX 세션
  빌드가 오래 걸리는 환경에서는 그동안 다른 기능까지 함께 멈췄고, 로드가 막혀 panic 이 나는
  환경에서는 명령이 통째로 죽었다. 이제 락 밖에서 최대 20초 기다리고, 그래도 준비되지 않으면
  이미지 파일에 한해 원인을 담은 오류를 돌려준다(PDF 는 종전대로 kordoc 이 처리).
- **인덱싱 도중 OCR 이 준비돼도 그 배치엔 반영되지 않던 문제 해소** — 인덱싱 시작 시점에 엔진이
  아직 없으면 **그 배치는 마지막 파일까지 OCR 없이** 돌았다. 워밍업은 보통 수백 ms 면 끝나는데,
  대량 폴더를 인덱싱하던 중이면 통째로 다시 인덱싱해야 했다. 감시 폴더 경로가 쓰던 공유 방식
  (`OnceCell` 공유)을 인덱싱·동기화 경로에도 적용해, 준비되는 즉시 **남은 파일부터** 반영된다.
- **설정을 저장할 때마다 OCR 재시도 제한이 풀리던 문제 해소** — 설정 창은 300ms 자동저장이라
  아무 항목이나 건드리면 저장이 들어오는데, 그때마다 v3.4.7 이 넣은 재시도 상한이 초기화됐다.
  이제 OCR 을 **방금 켠 경우에만** 초기화한다.

### 🔧 내부

- `.gitattributes` 에 `*.dll`/`*.dylib`/`*.so` binary 지정 — v3.4.4 부터 설치본 번들을 앱이 직접
  로드하므로 체크아웃 변환이 섞이면 곧장 기동 실패다(dict.txt 사례의 예방 확장).
- 번들 무결성 테스트 확대: `onnxruntime.dll` 해시(Windows), `pdfium.dll` 잘림, seed 멱등성.
- `scripts/setup-macos-resources.sh` 가 0바이트 placeholder 를 실물로 착각하던 판정(`-f` → `-s`)
  수정 + 미실행 시 빌드가 왜 깨지는지 헤더에 명시.
- 워밍업 스레드가 실패 카운터를 올리기 **전** 에 진행 플래그를 내리던 순서 수정 — 그 틈에 들어온
  요청이 아직 0인 카운터를 보고 통과해 자동 재시도 상한을 넘길 수 있었다. 같은 레이스 때문에
  회귀 테스트도 Windows 에서 간헐 실패해, 워밍업 "흔적"(진행 중·준비됨·실패 누적)을 폴링하도록
  고쳤다.
- 폐쇄망 스위치 `DOCUFINDER_OFFLINE` 을 README·인앱 도움말에 문서화 (종전엔 CHANGELOG 에만 있어
  해당 환경 사용자가 찾을 수 없었다).

## [3.4.7] - 2026-07-24

**ONNX Runtime 로드가 막힌 환경 대응 — 원인 진단 + 재시도 폭주 차단 (이슈 #35 후속)**

v3.4.6 사용자 로그로 이 환경의 진짜 벽이 확인됐습니다. 이 PC 에서는 **ONNX Runtime 네이티브
라이브러리 자체가 로드되지 않습니다** — 앱의 Rust 경로(panic)와 kordoc 사이드카의 Node 경로
(`ERR_DLOPEN_FAILED: DLL initialization routine failed`)가 서로 다른 로더인데도 똑같이 실패합니다.

### 🐛 수정

- **워밍업 재시도 폭주 차단** — v3.4.6 은 "실패해도 다음 기회에 재시도"가 목표였는데, 로드가
  아예 막힌 환경에서는 인덱싱마다 다시 시도해 같은 경고가 수십 줄 쌓였다. 자동 재시도를 3회로
  제한하고, 설정에서 OCR 을 다시 켜면 초기화되도록 했다.
- **로드 실패 원인 진단 추가(Windows)** — 실패 시 ONNX Runtime 과 그 전제인 VC++ 재배포 런타임을
  직접 한 번 더 열어 `GetLastError` 를 남긴다. 오류 번호로 "보안솔루션 차단(5) / 의존 런타임
  누락(126) / DLL 초기화 실패(1114) / 파일 손상(193)" 을 구분해 조치를 안내한다.
- **레이아웃 분석 모델 안내 문구 수정** — 인터넷이 막힌 환경에서 OCR 을 켜면 이 선택 기능의
  모델을 받지 못해 "다운로드 실패" 오류가 떴다. OCR 본체는 정상 동작한다는 점이 드러나도록
  문구와 강조 수준을 고쳤다.

## [3.4.6] - 2026-07-24

**OCR 을 설정에서 켜면 재시작 전까지 동작하지 않던 문제 해소 (이슈 #35 후속)**

### 🐛 수정

- **설정에서 OCR 을 켜도 OCR 이 돌지 않던 문제 해소** — v3.4.5 는 UI 가 OCR 초기화에 굶지 않도록
  초기화 호출을 전부 "준비됐으면 쓴다"로 바꿨는데, 그 결과 **엔진을 실제로 만드는 경로가 앱 시작
  워밍업 하나뿐**이 됐다. 그래서 `ocr_enabled: false` 로 시작한 앱(3.4.4 이전 우회 설정)에서 OCR 을
  켜면 재시작 전까지 스캔 문서·이미지 OCR 이 전혀 동작하지 않았고, OCR 관련 로그도 한 줄도 남지
  않았다. 이제 **설정 토글·인덱싱 시점에도 워밍업이 걸리고, 실패해도 다음 기회에 재시도**한다.
- **시작 워밍업이 번들 모델 적용(seed)과 겹치던 레이스 해소** — seed 는 해시가 다른 파일을 지웠다
  다시 복사하는데, 그 사이에 OCR 초기화가 사전을 읽으면 그 세션 내내 OCR 이 죽었다. 워밍업을 모델
  준비가 끝난 뒤로 옮겼다.
- **OCR 사전(dict.txt)이 Windows 설치본에서 개행 변환으로 오염되던 문제 해소** — 체크아웃 시
  LF→CRLF 로 바뀌어 파일 크기가 47,451 → 59,396 바이트가 됐고, 그 탓에 **부팅할 때마다 사전을
  삭제·재복사**(위 레이스를 매번 유발)하고 무결성 검증 경로는 항상 실패했다. `.gitattributes` 로
  변환을 막고, 번들 리소스 해시가 선언값과 일치하는지 CI 에서 검사한다.
- **폐쇄망에서 OCR 을 켜면 곧장 "모델 다운로드 실패" 가 뜨던 문제 해소** — 모델이 비어 있을 때
  설치본 번들을 먼저 적용하고, 그래도 없을 때만 다운로드를 시도한다.

### 🔧 내부

- `AppContainer::spawn_ocr_warmup` — 멱등·논블로킹 워밍업. 컨테이너 락을 잡지 않으며 실패 시
  진행 플래그를 되돌려 재시도 가능하게 둔다.
- 회귀 테스트 3종 추가: 워밍업 재시도, 인덱싱 경로의 워밍업 트리거, 번들 리소스 해시 일치.

## [3.4.5] - 2026-07-23

**OCR 초기화가 앱 전체를 멈추게 하던 문제 해소 — 폴더 목록이 비어 보이던 증상 (이슈 #35 후속)**

### 🐛 수정

- **감시 폴더 목록이 통째로 비어 보이던 문제 해소** — v3.4.4 에서 앱은 정상 기동하게 됐으나,
  OCR 엔진(ONNX 세션) 초기화가 오래 걸리는 환경에서 폴더 트리가 뜨지 않고
  `get_folders_with_info` 가 반복 타임아웃됐다. 원인은 OCR 엔진이 감시 매니저와 같은 지연 초기화
  셀을 공유해, OCR 초기화가 끝날 때까지 폴더 목록 조회까지 함께 대기한 것. **인덱싱 데이터는
  손상되지 않았고**, 조회만 막혀 비어 보인 것이다.
- 이제 OCR 엔진 초기화는 **전용 백그라운드 워밍업**에서만 수행하고, 나머지 경로는 준비된 경우에만
  가져다 쓴다. OCR 초기화가 지연되거나 실패해도 폴더 목록·검색·인덱싱은 그대로 동작하며,
  스캔 문서 OCR 만 비활성된다.

### 🔧 내부

- OCR 엔진을 `Arc<OnceCell>` 공유로 전환 — 임베더·벡터 인덱스와 동일 패턴. 감시 매니저 생성 시점에
  아직 워밍업 전이어도 준비되는 즉시 반영되어, 생성 순서에 따라 OCR 이 꺼진 채 굳지 않는다.
- OCR 초기화에 단계별 로그(`[1/3] det 세션` → `[2/3] rec 세션` → `[3/3] 사전`) 추가 — 환경별로
  어느 단계에서 지연되는지 로그만으로 판별 가능.
...
- **OCR을 켜면 앱이 여전히 기동하지 않던 문제 해소 (이슈 #35 후속)** — v3.4.3에서
  ...
  하는 dropper 패턴으로 오인해 프로세스를 격리한 것. 이제 `onnxruntime.dll`·`pdfium.dll`을
...
- **OCR 활성 시 앱이 기동하지 않던 문제 해소 (이슈 #35)** — 스캔/이미지 PDF
  래스터화에 쓰는 pdfium 동적 라이브러리만 설치본에서 빠져 있어, OCR을 켜면 앱이
  실행 중 GitHub에서 pdfium을 내려받으려 했다. 망분리(폐쇄망) 환경에서는 이 다운로드가
  막혀 기동이 멈췄고, 내부망 보안솔루션(APT 대응 제품)은 "실행 중 외부 바이너리
  다운로드"를 악성(dropper) 행위로 오인해 프로세스를 격리했다. 이제 pdfium을 설치본에
  함께 배포해 실행 중 다운로드를 없앴다.

### 🔧 내부

- **오프라인 강제 스위치 `DOCUFINDER_OFFLINE`** — `1`/`true` 로 설정하면 모든 런타임
  모델·라이브러리 다운로드를 차단한다. 번들 파일 seed 실패 같은 예외 상황에서도
  폐쇄망에서 네트워크를 건드리지 않게 하는 안전장치(설치본은 필수 자산을 모두 번들하므로
  평상시엔 다운로드 경로 자체를 타지 않는다).
- Windows/macOS 빌드 모두 pdfium(win: `pdfium.dll`, mac: `libpdfium.dylib`)을 번들
  리소스에 포함.

## [3.4.2] - 2026-07-19

**대형 문서 인덱싱 수정 + 파싱 품질 (kordoc v4.2.3)**

### 🐛 수정

- **대형 HWPX 인덱싱 실패 해소** — 단일 섹션 XML이 수십 MB에 달하는 대형 실문서가
  ZIP bomb으로 오인되어 인덱싱에서 통째로 빠지던 것 수정 (kordoc ZIP 해제 한도
  100→256MB, 1만 건 실문서 서베이 실측 반영).
- **서식 문서 읽기 순서 보존** — 한 문단·한 셀 안에서 표와 텍스트가 번갈아 놓인
  서식(기간 입력란 `[날짜] 부터 [날짜] 까지`, 결재문서 두문 등)이 원문 배치
  순서대로 인덱싱된다. 종전에는 텍스트가 표 앞으로 끌려나와 스니펫·구문 검색이
  어긋났다 (kordoc #49·#50).

### ✨ 추가

- **취소선 추출** — 법령 개정문 등의 삭제 표시가 `~~취소선~~`으로 구분되어
  인덱싱된다 (HWP5·HWPX).

### 🔧 내부

- kordoc 사이드카 SHA핀 v4.2.1 → v4.2.3(a7a2a2d) 갱신.

## [3.4.1] - 2026-07-19

**이미지 파일을 kordoc 직접 입력으로 인덱싱 — 표 서식 이미지도 표 구조 복원 (kordoc v4.2.1)**

### ✨ 추가

- **이미지(PNG/JPG/WebP) 인덱싱을 kordoc 에 우선 위임** — OCR 옵션이 켜져 있으면
  이미지 파일을 kordoc 직접 입력(v4.2.1 신기능)으로 파싱한다. 페이지 픽셀에서
  표 괘선을 감지해 병합셀 표까지 마크다운 표로 복원되므로, 서식 스크린샷·스캔
  이미지가 표 구조 그대로 검색·미리보기에 들어온다. kordoc 실패 시 자체
  PaddleOCR 엔진으로 fallback (BMP/TIFF 는 종전대로 자체 엔진 전용).
- **WebP 인덱싱 지원** — OCR 대상 이미지 확장자에 webp 추가 (kordoc 경로 전용).

### 🔧 변경

- kordoc 사이드카 SHA 핀 갱신 (v4.2.0 → v4.2.1) — 이미지 직접 입력 + 래스터
  괘선 감지 반영.
- 이미지 파일에서 kordoc 만 시도하고 실패한 경우("OCR로 다시 읽기" 등) generic
  "Unsupported file type" 대신 kordoc 의 실제 에러를 보존해 원인 진단 가능.

## [3.4.0] - 2026-07-17

**스캔·깨진 PDF OCR 통합 — kordoc v4.2.0 내장 OCR 위임 + "OCR로 다시 읽기"**

### ✨ 추가

- **"OCR로 다시 읽기" (우클릭 메뉴)** — 검색 결과의 PDF·이미지 파일을 강제 OCR 로
  단건 재인덱싱한다 (신규 `reindex_file` 커맨드). 스캔본이나 '복사 시 깨짐' 배지가
  뜬 문서를 클릭 한 번으로 다시 읽어 검색 인덱스를 갱신. 전역 OCR 토글과 무관하게
  동작하며, 첫 사용 시 kordoc OCR 모델(~18MB)이 자동 다운로드된다.
- **PDF 텍스트 OCR 을 kordoc 내장 엔진(PP-OCRv5 korean, v4.2.0)에 위임** — OCR 켜짐
  시 PDF 파싱이 kordoc `--ocr` 로 실행된다. 품질 신호(스캔·글꼴 매핑 깨짐)가 가리키는
  페이지만 정밀 인식하고 정상 페이지는 그대로 두며, OCR 라인 좌표가 표 감지
  파이프라인을 타므로 **스캔본에서도 표 구조가 복원**된다. 정상 PDF 에는 비용 0.
  kordoc 실패 시 기존 Rust 파서+자체 OCR 로 폴백 (이미지 파일 OCR 은 종전대로 자체 엔진).

### 🐛 수정

- **깨진 텍스트층(CID 오매핑) PDF 가 OCR 없이 쓰레기 텍스트로 인덱싱되던 문제** —
  종전에는 kordoc 이 "성공"으로 돌려준 깨진 본문을 그대로 저장해 OCR 이 영영 걸리지
  않았다. kordoc 의 페이지 품질 신호(garbled_hangul·high_pua 등)로 해당 페이지만
  OCR 재인식.
- **'복사 시 깨짐' 배지가 OCR 적용 문서에서 사라지던 문제** — OCR 이 본문을 대체하면
  인덱스 텍스트가 깨끗해져 배지 판정이 놓쳤다. 파서의 원본 품질 힌트(`garbled_hint`)를
  판정에 반영 — 원본을 열어 복사하면 여전히 깨진다는 사실이 배지로 유지된다.
- **kordoc 연동의 stderr 문자열 매칭 제거** — "이미지 기반 PDF" 문자열 grep 대신
  구조화 경고 코드(NEEDS_OCR)·`isImageBased`·`pageQuality` JSON 필드를 소비한다
  (로케일/메시지 변경에 안 깨짐).

### 🔧 변경

- **kordoc 사이드카 SHA핀 v4.1.0+ci(350335b) → v4.2.0(4f68c6c)** — 내장 텍스트
  OCR(PP-OCRv5 korean) 신설, HWP3 구버전 파서 정합 3종(탭/필드코드/책갈피 스트림
  소비·사적영역 문자 매핑), 수식 OCR 페이지 off-by-one 수정(pdfium 0-based)이 앱 내
  문서 파싱에 반영된다. publish.yml 두 job 동일 SHA + 로컬 dev 번들 재생성.
- 텍스트 OCR 사용 시 kordoc 호출 타임아웃을 수식 OCR 과 동일하게 600초로 확장.
