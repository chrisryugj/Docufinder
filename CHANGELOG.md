...
기술부채 정리. 문서 변환기 kordoc 4.14.3 연동 준비(이미지 건너뛰기·상주 변환 워커), 검색·색인 결함 수리,
결과 목록·미리보기 개편, 화면 대비·글자 크기 정리. kordoc 4.14.3 발행 뒤 번들 핀을 올려 3.8.9 로 낸다.

### ⚡ 성능

- **[변환] 문서 속 그림 바이트를 받지 않음 (kordoc 4.14.3 번들 때 켜짐)**: 앱은 문서 속 그림을 쓰지 않는데
...
- **[변환] 상주 변환 워커 (kordoc 4.14.3 번들 때 켜짐)**: 파일마다 node 를 새로 띄우던 것을 워커 최대
